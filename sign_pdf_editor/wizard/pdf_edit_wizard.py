...
class SignPdfEditWizard(models.TransientModel):
    ...
    def _onchange_template_id(self):
        for wiz in self:
            wiz.page_count = wiz.template_id._get_page_count() if wiz.template_id else 0
    ...
    def _get_doc(self):
        ...
        attachment = self.template_id._get_main_attachment()
    ...
    def _save_doc(self, doc):
        ...
        attachment = self.template_id._get_main_attachment()
        if not attachment:
            raise UserError(_("Could not find the PDF attachment to save changes to."))
